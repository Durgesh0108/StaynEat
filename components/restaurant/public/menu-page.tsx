...
interface RestaurantMenuPageProps {
  ...
  initialTable: { id: string; tableNumber: string; capacity: number } | null;
}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          {!item.isAvailable ? (
            ...
            <button
              onClick={onAdd}
              className="flex items-center gap-1 bg-primary-50 dark:bg-primary-950 text-primary-600 dark:text-primary-400 border border-primary-200 dark:border-primary-800 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-primary-100 transition-colors"
            >
            ...
            <div className="flex items-center gap-2 bg-primary-50 dark:bg-primary-950 rounded-lg px-2 py-1">
              <button onClick={onRemove} className="text-primary-600 dark:text-primary-400">
                <Minus className="h-3 w-3" />
              </button>
              <span className="text-primary-700 dark:text-primary-300 font-semibold text-sm min-w-4 text-center">
                {quantity}
              </span>
              <button onClick={onAdd} className="text-primary-600 dark:text-primary-400">
                <Plus className="h-3 w-3" />
              </button>
            </div>
          )}
...
const STATUS_META: Record<string, { label: string; pill: string; bar: string }> = {
  ...
  READY:     { label: "Ready to serve",      pill: "bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-300",       bar: "bg-teal-400" },
...
function OrderTracker({ sessionId, businessId }: { sessionId: string; businessId: string }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 mb-3">
      <div className={cn("rounded-2xl overflow-hidden border shadow-sm", allDone ? "border-success-300 dark:border-success-700" : "border-amber-200 dark:border-amber-800")}>
        {/* Header */}
        <button
          onClick={() => setExpanded((e) => !e)}
          className={cn(
            "w-full flex items-center justify-between px-4 py-3",
            allDone ? "bg-success-50 dark:bg-success-900/20" : "bg-amber-50 dark:bg-amber-900/20"
          )}
        >
          <div className="flex items-center gap-2.5">
            <ChefHat className={cn("h-4 w-4", allDone ? "text-success-500" : "text-amber-500")} />
            <span className="font-semibold text-sm text-gray-900 dark:text-white">
              {allDone ? "All orders delivered!" : `${activeCount} order${activeCount !== 1 ? "s" : ""} in progress`}
            </span>
        ...
        {expanded && (
          <div className="bg-white dark:bg-gray-900 divide-y divide-gray-50 dark:divide-gray-800">
            {orders.map((order, idx) => {
              ...
              return (
                <div key={order.id} className="flex gap-0">
                  {/* Color side bar */}
                  <div className={cn("w-1 flex-shrink-0", meta.bar)} />
                  <div className="flex-1 px-4 py-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-medium text-gray-400">Round {idx + 1}</span>
                      <span className={cn("text-xs font-semibold px-2.5 py-0.5 rounded-full", meta.pill)}>
                        {meta.label}
                      </span>
                    </div>
...
export function RestaurantMenuPage({ business, initialTable }: RestaurantMenuPageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {initialTable && (
                  <p className="text-xs text-primary-600 dark:text-primary-400">Table {initialTable.tableNumber}</p>
                )}
              </div>
            </div>
            {cartCount > 0 && (
              <button
                ...
                className="relative flex items-center gap-2 bg-primary-500 text-white px-4 py-2 rounded-xl text-sm font-medium"
              >
      ...
      <div className="max-w-2xl mx-auto px-4 py-3 space-y-3">
        ...
        <div className="flex gap-2">
          {["all", "veg", "nonveg"].map((f) => (
            <button
              key={f}
              onClick={() => setVegFilter(f as typeof vegFilter)}
              className={cn(
            ...
            >
              ...
              {f === "nonveg" && <div className="w-2 h-2 rounded-full bg-red-500" />}
      ...
      {/* Live Order Tracker — shown above menu items */}
      {sessionId && (
                ...
                {items.map((cartItem) => (
                  <div key={cartItem.menuItem.id} className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800 rounded-lg px-2 py-1">
                      <button onClick={() => {
                        if (cartItem.quantity > 1) updateQuantity(cartItem.menuItem.id, cartItem.quantity - 1);
                        else removeItem(cartItem.menuItem.id);
                      }}>
                    ...
                    <span className="text-sm font-semibold w-16 text-right text-gray-900 dark:text-white">
